...
const ROLLBACK_FILE = path.join(__dirname, "rollback_020_goals_coach_member_conversation_provider_coaching_replay.sql");
...
function migrationOrdinal(version) {
  ...
  if (!match || match[1] === "000") throw new Migration020Error("noncanonical_migration_version");
...
async function runRollback(options = {}) {
  if (!options.skipConfirmation
    && process.env.CONFIRM_GOALS_COACH_MEMBER_CONVERSATION_PROVIDER_COACHING_REPLAY_ROLLBACK !== "YES") {
    throw new Migration020Error("confirmation_required");
  }
  ...
  try {
    ...
    const transaction = await runBoundedPostgresTransaction({
      ...
      async work({ query, remainingMilliseconds }) {
        ...
        if (applied.rows[0].checksum !== checksum(fs.readFileSync(MIGRATION_FILE, "utf8"))) {
          throw new Migration020Error("checksum_mismatch");
        }
        ...
        if (ledger.rows.some((row) => migrationOrdinal(row.version) > currentOrdinal)) {
          throw new Migration020Error("later_migration_applied");
        }
        await query("LOCK TABLE goals_coach_member_conversation_provider_coaching_replays IN ACCESS EXCLUSIVE MODE");
        const rows = await query(
          "SELECT 1 FROM goals_coach_member_conversation_provider_coaching_replays LIMIT 1"
        );
        if (rows.rows.length) throw new Migration020Error("provider_coaching_replay_rows_exist");
    ...
    throw new Migration020Error("rollback_failed", error);
  } finally {
...
if (require.main === module) {
  runRollback()
    ...
    .catch(() => {
      console.error("[UGF] Migration 020 rollback is unavailable for this database state");
      process.exitCode = 1;
    });
}
