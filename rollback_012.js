...
const { CONNECTION_MILLISECONDS, MIGRATION_VERSION, OVERALL_MILLISECONDS, Migration012Error, checksum } = require("./migrate_012");
const MIGRATION_FILE = path.join(__dirname, "migration_012_goals_coach_safety_intake_v2.sql");
const ROLLBACK_FILE = path.join(__dirname, "rollback_012_goals_coach_safety_intake_v2.sql");
...
async function runRollback(options = {}) {
  if (!options.skipConfirmation && process.env.CONFIRM_GOALS_COACH_SAFETY_INTAKE_V2_ROLLBACK !== "YES") throw new Migration012Error("confirmation_required");
  ...
  const now = options.monotonicNow || monotonicNow; const terminalState = createTerminalState();
  ...
  try {
    ...
    const transaction = await runBoundedPostgresTransaction({ pool: { connect: async () => client }, terminalState, monotonicNow: now, outerDeadlineNs, phaseMilliseconds: options.overallMilliseconds || OVERALL_MILLISECONDS, async work({ query, remainingMilliseconds }) {
      ...
      if (applied.rows[0].checksum !== checksum(fs.readFileSync(MIGRATION_FILE, "utf8"))) throw new Migration012Error("checksum_mismatch");
      ...
      if (later.rows.length) throw new Migration012Error("later_migration_applied");
      const count = await query("SELECT COUNT(*)::int AS count FROM goals_coach_member_safety_intake_v2_assessments");
      if (count.rows[0].count !== 0) throw new Migration012Error("immutable_rows_present");
    ...
    for (let cause = error; cause; cause = cause.cause) if (cause instanceof Migration012Error) throw cause;
    throw new Migration012Error("rollback_failed", error);
  }
