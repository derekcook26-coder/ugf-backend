...
async function loadBoundItems(client, row) {
  ...
  const items = (await client.query("SELECT id,exercise_name,prescription_json FROM coach_plan_exercises WHERE plan_id=$1 AND id=ANY($2::bigint[])", [row.plan_id, ids])).rows;
  ...
  if (!safety) state = "SAFETY_REQUIRED";
    ...
    if (!consent) state = "CONSENT_REQUIRED";
      ...
      if (!plan) state = "UNAVAILABLE";
        ...
        else { const bound = await loadBoundItems(client, original); const selected = bound && String(original.option_item_ids[input.continuation.optionId]); item = bound && (await client.query("SELECT id,exercise_name,prescription_json FROM coach_plan_exercises WHERE id=$1 AND plan_id=$2", [selected, original.plan_id])).rows[0]; if (!bound || !item || itemHash(item) !== original.option_item_hashes[input.continuation.optionId]) { state = "UNAVAILABLE"; plan = null; item = null; original = null; } else { state = "READY"; await client.query("UPDATE goals_coach_member_today_attempts SET consumed_at=NOW() WHERE id=$1 AND consumed_at IS NULL", [original.id]); } }
      } else {
