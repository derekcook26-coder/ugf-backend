...
const { MIGRATION_VERSION, checksum } = require("./migrate_009");

const ROLLBACK_FILE = path.join(
  __dirname,
  "rollback_009_goals_coach_member_safety_intake.sql"
);
const MIGRATION_FILE = path.join(
  __dirname,
  "migration_009_goals_coach_member_safety_intake.sql"
);
const MIGRATION_LOCK_KEY = 82720509;
...
async function runRollback(options = {}) {
  if (
    ...
    && process.env.CONFIRM_GOALS_COACH_MEMBER_SAFETY_INTAKE_ROLLBACK !== "YES"
  ) {
    throw new Error(
      "Set CONFIRM_GOALS_COACH_MEMBER_SAFETY_INTAKE_ROLLBACK=YES "
      + "to run this destructive rollback"
    );
  }
  ...
  try {
    ...
    if (applied.rows[0].checksum !== migrationChecksum) {
      throw new Error("Cannot roll back migration 009 with a different checksum");
    }
    ...
    if (later.rows.length) {
      throw new Error(
        `Cannot roll back migration 009 while later migration ${later.rows[0].version} is applied`
      );
    }
