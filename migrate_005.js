...
const MIGRATION_VERSION = "005_goals_coach_voice_transcription_provenance";
const REQUIRED_MIGRATION_VERSION = "004_goals_coach_workout_state_and_turn_provenance";
...
async function runMigration(options = {}) {
  ...
  try {
    ...
    if (!ledger.rows[0].name) {
      throw new Error("Migration 004 must be applied before Migration 005");
    }
    ...
    if (existing.rows.length) {
      if (existing.rows[0].checksum !== sqlChecksum) {
        throw new Error("Migration 005 was already applied with a different checksum");
      }
