...
const MIGRATION_VERSION = "009_goals_coach_member_safety_intake";
const REQUIRED_MIGRATION_VERSION = "008_goals_coach_owner_editable_workout_sessions";
...
async function runMigration(options = {}) {
  ...
  try {
    ...
    if (!required.rows.length) {
      throw new Error("Migration 008 must be applied before Migration 009");
    }
    ...
    if (existing.rows.length) {
      if (existing.rows[0].checksum !== sqlChecksum) {
        throw new Error("Migration 009 was already applied with a different checksum");
      }
