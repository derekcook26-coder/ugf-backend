...
const MIGRATION_VERSION = "008_goals_coach_owner_editable_workout_sessions";
const REQUIRED_MIGRATION_VERSION = "007_goals_coach_owner_workout_tracking";
const MIGRATION_FILE = path.join(
  __dirname,
  "migration_008_goals_coach_owner_editable_workout_sessions.sql"
);
const MIGRATION_LOCK_KEY = 82720508;
...
async function runMigration(options = {}) {
  ...
  try {
    ...
    if (!required.rows.length) throw new Error("Migration 007 must be applied before Migration 008");
    ...
    if (existing.rows.length) {
      if (existing.rows[0].checksum !== sqlChecksum) {
        throw new Error("Migration 008 was already applied with a different checksum");
      }
