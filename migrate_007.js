...
const MIGRATION_VERSION = "007_goals_coach_owner_workout_tracking";
const REQUIRED_MIGRATION_VERSION = "006_goals_coach_safety_review_routing";
const MIGRATION_FILE = path.join(__dirname, "migration_007_goals_coach_owner_workout_tracking.sql");
const MIGRATION_LOCK_KEY = 82720507;
...
async function runMigration(options = {}) {
  ...
  try {
    ...
    if (!required.rows.length) throw new Error("Migration 006 must be applied before Migration 007");
    ...
    if (existing.rows.length) {
      if (existing.rows[0].checksum !== sqlChecksum) {
        throw new Error("Migration 007 was already applied with a different checksum");
      }
