...
const { MIGRATION_VERSION } = require("./migrate_004");

const ROLLBACK_FILE = path.join(__dirname, "rollback_004_goals_coach_workout_state_and_turn_provenance.sql");
const MIGRATION_LOCK_KEY = 82720404;
function databaseSsl() { return process.env.PGSSLMODE === "disable" ? false : { rejectUnauthorized: false }; }
function createPool(connectionString) {
  if (!connectionString) throw new Error("DATABASE_URL is required");
  return new Pool({ connectionString, ssl: databaseSsl(), max: 1 });
}
async function runRollback(options = {}) {
  if (!options.skipConfirmation && process.env.CONFIRM_PHASE1B_ROLLBACK !== "YES") {
    throw new Error("Set CONFIRM_PHASE1B_ROLLBACK=YES to run this destructive rollback");
  }
  ...
  try {
    ...
    if (!ledger.rows[0].name) { await client.query("ROLLBACK"); return { status: "not_applied", version: MIGRATION_VERSION }; }
    const applied = await client.query("SELECT 1 FROM app_schema_migrations WHERE version = $1", [MIGRATION_VERSION]);
    if (!applied.rows.length) { await client.query("ROLLBACK"); return { status: "not_applied", version: MIGRATION_VERSION }; }
    ...
    if (later.rows.length) throw new Error(`Cannot roll back migration 004 while later migration ${later.rows[0].version} is applied`);
...
if (require.main === module) {
  runRollback().then((result) => console.log(`[UGF] Migration ${result.version}: ${result.status}`)).catch((error) => {
