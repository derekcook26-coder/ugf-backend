...
const ROLLBACK_FILE = path.join(__dirname, "rollback_018_goals_coach_member_conversation_turn_idempotency.sql");
...
function migrationOrdinal(version) {
  ...
  if (!match || match[1] === "000") throw new Migration018Error("noncanonical_migration_version");
...
async function runRollback(options = {}) {
  if (!options.skipConfirmation
    && process.env.CONFIRM_GOALS_COACH_MEMBER_CONVERSATION_TURN_IDEMPOTENCY_ROLLBACK !== "YES") {
    throw new Migration018Error("confirmation_required");
  }
  ...
  try {
    ...
    const transaction = await runBoundedPostgresTransaction({
      ...
      async work({ query, remainingMilliseconds }) {
        ...
        if (applied.rows[0].checksum !== checksum(fs.readFileSync(MIGRATION_FILE, "utf8"))) {
          throw new Migration018Error("checksum_mismatch");
        }
        ...
        if (ledger.rows.some((row) => migrationOrdinal(row.version) > currentOrdinal)) {
          throw new Migration018Error("later_migration_applied");
        }
        await query("LOCK TABLE goals_coach_member_conversation_turn_idempotency IN ACCESS EXCLUSIVE MODE");
        const rows = await query("SELECT 1 FROM goals_coach_member_conversation_turn_idempotency LIMIT 1");
        if (rows.rows.length) throw new Migration018Error("idempotency_rows_exist");
    ...
    throw new Migration018Error("rollback_failed", error);
  } finally {
...
if (require.main === module) {
  runRollback()
    ...
    .catch(() => {
      console.error("[UGF] Migration 018 rollback is unavailable for this database state");
      process.exitCode = 1;
    });
}
