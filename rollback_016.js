...
const { CONNECTION_MILLISECONDS, MIGRATION_FILE, MIGRATION_VERSION, OVERALL_MILLISECONDS, Migration016Error, checksum } = require("./migrate_016");
const ROLLBACK_FILE = path.join(__dirname, "rollback_016_goals_coach_adaptive_safety_intake.sql");
...
function migrationOrdinal(version) {
  ...
  if (!match || match[1] === "000") throw new Migration016Error("noncanonical_migration_version");
  return Number(match[1]);
}
function createPool(connectionString, environment = process.env) {
  if (!connectionString) throw new Migration016Error("database_url_required");
...
async function runRollback(options = {}) {
  if (!options.skipConfirmation && process.env.CONFIRM_GOALS_COACH_ADAPTIVE_SAFETY_ROLLBACK !== "YES") throw new Migration016Error("confirmation_required");
  ...
  try {
    ...
    const tx = await runBoundedPostgresTransaction({ preAcquiredClient: client, terminalState, monotonicNow: now, outerDeadlineNs, phaseMilliseconds: options.overallMilliseconds || OVERALL_MILLISECONDS, async work({ query, remainingMilliseconds }) {
      ...
      if (applied.rows[0].checksum !== checksum(fs.readFileSync(MIGRATION_FILE, "utf8"))) throw new Migration016Error("checksum_mismatch");
      ...
      if (ledger.rows.some((row) => migrationOrdinal(row.version) > currentOrdinal)) throw new Migration016Error("later_migration_applied");
      const v3 = await query("SELECT 1 FROM goals_coach_member_safety_intake_v2_assessments WHERE notice_version='GC-MEMBER-SAFETY-NOTICE-3' OR rule_version='GC-MEMBER-SAFETY-INTAKE-3' LIMIT 1");
      if (v3.rows.length) throw new Migration016Error("v3_rows_exist");
    ...
    for (let cause = error; cause; cause = cause.cause) if (cause instanceof Migration016Error) throw cause;
    throw new Migration016Error("rollback_failed", error);
  } finally { if (ownsPool) await pool.end(); }
}
if (require.main === module) runRollback().then((result) => console.log(`[UGF] Migration ${result.version}: ${result.status}`)).catch(() => { console.error("[UGF] Migration 016 rollback is unavailable for this database state"); process.exitCode = 1; });
