...
const { MIGRATION_VERSION } = require("./migrate_002");

const ROLLBACK_FILE = path.join(__dirname, "rollback_002_goals_coaching_foundation.sql");
...
async function runRollback(options = {}) {
  if (!options.skipConfirmation && process.env.CONFIRM_PHASE2_ROLLBACK !== "YES") {
    throw new Error("Set CONFIRM_PHASE2_ROLLBACK=YES to run this destructive rollback");
  }
  ...
  try {
    ...
    const table = await client.query("SELECT to_regclass('public.app_schema_migrations') AS name");
    ...
    if (later.rows.length > 0) {
      throw new Error(`Cannot roll back migration 002 while later migration ${later.rows[0].version} is applied`);
    }
